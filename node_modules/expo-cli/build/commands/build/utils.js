...
async function checkIfSdkIsSupported(sdkVersion, platform) {
  ...
  const versionWillNeverBeSupported = _xdl().Versions.gteSdkVersion({
    sdkVersion
  }, '47.0.0');
  if (!isSupported) {
    if (versionWillNeverBeSupported) {
      _log().default.error(_chalk().default.red(`The Classic Build service is deprecated. The final version to support it was Expo SDK 46.\n\nPlease migrate to EAS Build to continue building your app.\n${(0, _TerminalLink().learnMore)('https://docs.expo.dev/build-reference/migrating/')}`));
    } else {
