...
async function mergeSourceDirectoriresAsync(projectRoot, mergeSrcDirs, options) {
  ...
  _log().default.nested(`Starting project merge of ${srcDirs} into ${options.outputDir}`);

  // Merge app distributions
  await (0, _mergeAppDistributions().mergeAppDistributions)(projectRoot, [...mergeSrcDirs, options.outputDir],
  // merge stuff in srcDirs and outputDir together
  options.outputDir);
...
async function collectMergeSourceUrlsAsync(projectRoot, mergeSrcUrl) {
  ...
  if (mergeSrcUrl.length > 0) {
    ...
    await _fsExtra().default.ensureDir(tmpFolder);
    ...
    const downloadDecompressPromises = mergeSrcUrl.map(async url => {
      ...
      await (0, _Tar().downloadAndDecompressAsync)(url, tmpFolderUncompressed);
...
async function actionAsync(projectRoot, options) {
  ...
  // Merge src dirs/urls into a multimanifest if specified
  const mergeSrcDirs = await collectMergeSourceUrlsAsync(projectRoot, options.mergeSrcUrl);
