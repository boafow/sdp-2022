...
async function actionAsync(projectRoot, options) {
  ...
  if (result.queryResult && result.queryResult.length > 0) {
    ...
    const hasSdkVersion = result.queryResult.some(publication => !!publication.sdkVersion);

    // Print info specific to each publication
    const headers = ['publishedTime', 'appVersion', ...(hasSdkVersion ? ['sdkVersion'] : []), ...(hasRuntimeVersion ? ['runtimeVersion'] : []), 'platform', 'channel', 'publicationId'];
    ...
    const resultRows = result.queryResult.map(publication => ({
      ...publication,
