...
async function actionAsync(projectRoot, options) {
  ...
  await (0, _spawnGradleAsync().assembleAsync)({
    ...props,
    androidProjectPath
  });
  ...
  if (apkFile) {
    ...
    await (0, _spawnGradleAsync().installAsync)({
      ...props,
      androidProjectPath
    });
  }
...
async function getInstallApkNameAsync(device, {
  ...
  _log().default.debug('Searching for APK: ' + apkVariantDirectory);
