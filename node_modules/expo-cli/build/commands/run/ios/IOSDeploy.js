...
async function installOnDeviceAsync(props) {
  ...
  const args = ['--bundle', bundle, '--id', udid, '--justlaunch',
  ...
  const output = await spawnIOSDeployAsync(args, message => {
    ...
    if (phaseMatch) {
      ...
      let phase = (_phaseMatch$ = phaseMatch[1]) === null || _phaseMatch$ === void 0 ? void 0 : (_phaseMatch$$trim = _phaseMatch$.trim) === null || _phaseMatch$$trim === void 0 ? void 0 : _phaseMatch$$trim.call(_phaseMatch$);
      // Remap name
      ...
      if (indicator) {
        if (currentPhase === 'Installing') {
          const copiedMessage = _chalk().default.gray`Copied ${copyingFileCount} file(s)`;
