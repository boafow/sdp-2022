...
async function ensureTypeScriptSetupAsync(projectRoot) {
  ...
  // Don't prompt in CI
  _commander().default.nonInteractive);
...
async function shouldSetupTypeScriptAsync(projectRoot) {
  const tsConfigPath = await (0, _resolveModules().hasTSConfig)(projectRoot);
...
async function ensureRequiredDependenciesAsync(projectRoot, skipPrompt = false) {
  ...
  if (!skipPrompt) {
    if (await (0, _prompts().confirmAsync)({
      message: (0, _wrapAnsi().default)(title + ` Would you like to install ${_chalk().default.cyan(readableMissingPackages)}?`,
